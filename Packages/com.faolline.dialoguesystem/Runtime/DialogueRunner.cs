...
namespace com.faolline.dialoguesystem
{
    ...
    public sealed class DialogueRunner
    {
        ...
        private DialogueStep EnterNode(string nodeId)
        {
            ...
            Debug.Log($"[DialogueRunner] EnterNode: {node.id} ({node.type}), speaker Key { node.speakerKey}");
            var step = new LineStep(node.id, node.speakerKey, node.speakerExpressionKey, node.textKey);
            Debug.Log($"[DialogueRunner] Line: '{step.textKey}' (speaker='{step.speakerKey}', mood={step.expressionKey})");
