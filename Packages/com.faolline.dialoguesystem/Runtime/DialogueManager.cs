...
namespace com.faolline.dialoguesystem
{
    ...
    public class DialogueManagerUserControlled : MonoBehaviour
    {
        ...
        private void HandleLine(LineStep s)
        {
            ...
            Debug.Log($"[Dialogue] Line: '{s.textKey}' (speaker='{s.speakerKey}', mood={s.expressionKey})");
            ui?.ShowLine(s);
        }
        ...
        private void OnGUI()
        {
            ...
            if (_lastLine != null)
            {
                GUI.Label(new Rect(x, y, w, h),
                    $"LINE — key='{_lastLine.textKey}' (speaker='{_lastLine.speakerKey}', mood={_lastLine.expressionKey})",
                    styleMono); y += h;
