...
namespace com.faolline.dialoguesystem
{
    public class SentenceInspectorView : VisualElement
    {
        ...
        private PopupField<string> _expressionField; // NEW
        ...
        public SentenceInspectorView()
        {
            ...
            _expressionField = new PopupField<string>("Expression", new List<string> { "neutral" }, 0)
            { name = "ExpressionPopup" };
            _expressionField.tooltip = "Clé d'expression/pose du speaker (ex: neutral, happy, thinking).";
            _expressionField.RegisterValueChangedCallback(OnExpressionChanged);
            _rootForm.Add(_expressionField);
            ...
            { name = "TextKeyPopup" };
            _textKeyField.tooltip = "Clé de traduction de la phrase.";
        ...
        public void ClearInspector()
        {
            ...
            _expressionField?.SetValueWithoutNotify("neutral");
        ...
        private void RebuildSpeakerChoices()
        {
            ...
            var desired = (_model != null && !string.IsNullOrEmpty(_model.speakerKey))
                ? _model.speakerKey
                : _speakerField.choices.FirstOrDefault();
            _speakerField.SetValueWithoutNotify(
                _speakerField.choices.Contains(desired) ? desired : _speakerField.choices.FirstOrDefault()
            );
        }
        ...
        private Speaker FindSpeakerByName(string name)
        {
            ...
            return _asset.Speakers.FirstOrDefault(s => s != null && s.SpeakerName == name);
        }

        private void RebuildExpressionChoices()
        {
            ...
            var speakerName = _speakerField != null ? _speakerField.value : _model?.speakerKey;
            var speaker = string.IsNullOrEmpty(speakerName) ? null : FindSpeakerByName(speakerName);

            List<string> keys;
            if (speaker != null && speaker.Expressions != null && speaker.Expressions.Count > 0)
                keys = speaker.Expressions.Select(e => e?.key ?? "").Where(k => !string.IsNullOrEmpty(k)).Distinct().ToList();
            else
                keys = new List<string> { "neutral" };

            if (keys.Count == 0) keys.Add("neutral");

            _expressionField.choices = keys;

            var desired = (_model != null && !string.IsNullOrEmpty(_model.speakerExpressionKey))
                ? _model.speakerExpressionKey
                : keys.FirstOrDefault() ?? "neutral";

            _expressionField.SetValueWithoutNotify(keys.Contains(desired) ? desired : keys[0]);
        }
        ...
        private void RebuildTextKeyChoices()
        {
            ...
            var desired = (_model != null && !string.IsNullOrEmpty(_model.textKey))
                ? _model.textKey
                : keys.FirstOrDefault();
        ...
        private void ApplyCommonValues()
        {
            if (_speakerField != null)
            {
                ...
                var desiredSpeaker = string.IsNullOrEmpty(_model.speakerKey) ? choices.FirstOrDefault() : _model.speakerKey;
        ...
        private void OnSpeakerChanged(ChangeEvent<string> evt)
        {
            ...
            _model.speakerKey = evt.newValue ?? string.Empty;

            // rafraîchir la liste d’expressions dispo pour ce speaker
            RebuildExpressionChoices();

            // applique la valeur affichée comme nouvelle valeur de modèle
            _model.speakerExpressionKey = _expressionField.value ?? "neutral";
        ...
        {
            if (_asset == null || _model == null) return;
            Undo.RecordObject(_asset, "Edit Expression");
            _model.speakerExpressionKey = evt.newValue ?? "neutral";
            EditorUtility.SetDirty(_asset);
            DialogueNode.BroadcastNodeDataChanged(_model.id);
        }
